...
class WebStubCntl {
	...
	function WebStubCntl() {
        ...
        $this->callerName = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'] : __FILE__; // 默认为这个
	}
	...
	function invoke(&$req, &$resp, $timeout = 3) {
		...
		if($ret != 0)
		{
            ...
            if ($itil_fail) exd_Attr_API2($itil_fail, 1);
            return $cmdid;
		}
        ...
        // 上报itil
        list($itil_success, $itil_fail, $itil_timeout) = $this->getItilId();
        if ($itil_success) exd_Attr_API2($itil_success, 1);
		return 0;
	}
	...
	function sendAndRecv(&$bs, &$bs2, $timeout) {
		...
		@socket_set_option($socket, SOL_SOCKET, SO_SNDTIMEO, array("sec"=>$timeout, "usec"=>$timeout));
		@socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array("sec"=>$timeout, "usec"=>$timeout));
		if(!$socket)
		{
		...
		if($ret != 0)
		{
            // 上报itil
            list($itil_success, $itil_fail, $itil_timeout) = $this->getItilId();
            if ($itil_timeout) exd_Attr_API2($itil_timeout, 1);
